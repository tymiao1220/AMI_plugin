...
export default class ModelsStack extends ModelsBase {
  ...
  /**
   * upPack current stack pixel data back to original bits array buffers
   */
  upPack(rawData) {
    const nbVoxels =
      this._dimensionsIJK.x * this._dimensionsIJK.y * this._dimensionsIJK.z;
  ...
  _packTo8Bits(channels, frame, textureSize, startVoxel, stopVoxel) {
    ...
    const frameDimension = frame[0].rows * frame[0].columns;  //672*672
    ...
    if (bitsAllocated === 8 && channels === 1 || bitsAllocated === 1) {
      ...
      for (let i = startVoxel; i < stopVoxel; i++) {
        ...
        // console.log(raw);
        // console.log(data[4 * coordinate + 2 * channelOffset]);
        // console.log(data[4 * coordinate + 2 * channelOffset] + 1);
        packIndex++;
  ...
  _unPackToOriginalBits(rawData, channels, frame, nbVoxels, startVoxel, stopVoxel) {
    ...
    if (bitsAllocated === 16 && channels === 1) {
      let data = new Uint16Array(nbVoxels);
      let coordinate = 0;
      let channelOffset = 0;

      for (let i = startVoxel; i < stopVoxel; i++) {
        let index_l = 4 * coordinate + 2 * channelOffset;
        let index_m = 4 * coordinate + 2 * channelOffset + 1;
        let least_significant = rawData[index_m];
        let most_significant = rawData[index_l];
      ...
      upPacked.textureType = THREE.RGBAFormat;
      upPacked.data = data;
    }
    return upPacked;
  }
