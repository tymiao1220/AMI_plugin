...
var ami = View.extend({
    ...
    initHelpersStack: function (rendererObj, stack, stack2 = null, refStackHelper = null, labelColor = undefined) {
        ...
        if (stack2 !== null) {
            ...
            for (let a = 0; a <= stack2.minMax[1]; a++) {
                ...
                a === 0
                    ? this.cursorOptionsDict['Clear Label'] = {
        ...
        if (inner.stackHelper === null) {
            ...
            inner.stackHelper.slice.intensityAuto = false;
            // set camera
    ...
    */
    ...
    annotationSelector: function (annotations, mode, editSegmentationFolderId = undefined, labelColor = this.params.color, cursorSize = 10) {
        if (mode === 'edit') {
            this.mode = 'edit';
            this.edit = true;
            ...
            let currrentSegItem = annotations.filter((x) => { if (x.folderId === editSegmentationFolderId) { return this; } })[0];

            this.editingAnnotation = this.stackFolder4.addFolder('Label:' + currrentSegItem.folderName);

            this.editingAnnotation.add(
                this.layerMix, 'opacity1', 0, 1).step(0.01).onChange(_.bind(function (value) {
                this.labelStack[currrentSegItem._id][3].uniformsLayerMix.uOpacity1.value = value;
            }, this));
            ...
            this.stackFolder4 = this.gui.addFolder('Labels');
            for (let a = 0; a < annotations.length; a++) {
                this.annotation[a] = this.stackFolder4.addFolder('Label' + a + ':' + annotations[a].folderName);
            ...
            case 'r3Editor_canvas':
                ...
                if (this.isDrawing) {
                    ...
                    if (this.r3Editor.boundingbox.left !== undefined) {
                        ...
                        let mouseTopLeft = {x: (this.r3Editor.boundingbox.left / this.r3Editor.context.canvas.clientWidth) * 2 - 1,
                            y: -(this.r3Editor.boundingbox.top / this.r3Editor.context.canvas.clientHeight) * 2 + 1};

                        let raycasterTopLeft = new THREE.Raycaster();
                        raycasterTopLeft.setFromCamera(mouseTopLeft, this.r3.camera);
                        let intersectsTopLeft = raycasterTopLeft.intersectObjects(this.r3.sceneLayer0.children, true);
                        this.r3Editor.ijk.topLeft =
                          CoreUtils.worldToData(this.stackHelper._stack.lps2IJK, intersectsTopLeft[0].point);

                        let mouseBottomRight = {x: (this.r3Editor.boundingbox.right / this.r3Editor.context.canvas.clientWidth) * 2 - 1,
                            y: -(this.r3Editor.boundingbox.bottom / this.r3Editor.context.canvas.clientHeight) * 2 + 1};
                        let raycasterBottomRight = new THREE.Raycaster();
                        raycasterBottomRight.setFromCamera(mouseBottomRight, this.r3.camera);
                        let intersectsBottomRight = raycasterBottomRight.intersectObjects(this.r3.sceneLayer0.children, true);
                        this.r3Editor.ijk.bottomRight =
                          CoreUtils.worldToData(this.stackHelper._stack.lps2IJK, intersectsBottomRight[0].point);
